  ...
  while (1) {
    if (tlen && (*p == '\0' || strchr(".[]:", *p))) {
      ...
      if (next == JSEL_ARRAY) {
        ...
        if (!json_type_matches(current, token[0]))
          goto notfound;
      } else {
        goto notfound;
      }
      ...
      if (next == JSEL_ARRAY) {
        ...
        if (len < 0 || len >= (int)sizeof(buf) ||
            (size_t)len > JSEL_MAX_TOKEN - tlen)
          goto notfound;
